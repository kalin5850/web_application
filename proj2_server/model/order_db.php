...
function get_order($order_id) {
    global $db;
    $query = 'SELECT * FROM orders WHERE orderID = :order_id order by orderID ';
    $statement = $db->prepare($query);
    $statement->bindValue(':order_id', $order_id);
    $statement->execute();
...
function update_delivery_status(){
    ...
    $query='UPDATE orders SET status = 2
            WHERE deliveryDay <= (SELECT dayNumber FROM systemday);
...
function get_undelivered_order(){
    ...
    $query ='SELECT * FROM orders
         INNER JOIN customers
         ON customers.customerID = orders.customerID
         WHERE status=1 and deliveryDay<>0 ';
    $statement = $db->prepare($query);
    $statement->execute();
    $orders = $statement->fetchAll();
    $statement->closeCursor();
...
function get_all_pizza_order() {
    global $db;
    $query = 'SELECT * FROM orders WHERE deliveryDay<>0 ';
    $statement = $db->prepare($query);
    $statement->execute();
    $order = $statement->fetchAll();
    $statement->closeCursor();
